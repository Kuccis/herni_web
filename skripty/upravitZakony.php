...
if(isset($_POST['submitUpravu']))
{
	if(empty($pravidlaNove)){
		header("Location: ../phpStranky/editorZakonu.php?error=zadnyText");
		exit();	//dokud neopravi chybu, kod pod timto ifem se neprovede
	}
	else {
		$sql = "UPDATE zakony SET text=?,datum=?,idUpravujiciho=? WHERE id=2";
		$stmt = mysqli_stmt_init($pripojeni);
  		if(!mysqli_stmt_prepare($stmt,$sql)){	//mysqli_stmt_prepare pripravuje sql prikaz k provedeni ale pokud nelze z nejakeho duvodu provest - vyhodime error
  			header("Location: ../phpStranky/editorZakonu.php?zmenaPravidel=neuspesna");
  			exit();
  		}
      else{
  			...
  			header("Location: ../phpStranky/editorZakonu.php?zmenaPravidel=uspesna");
  			exit();
  		}
    }
}
