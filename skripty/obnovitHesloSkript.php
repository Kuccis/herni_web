<?php
 if(isset($_POST["zmenitHeslo"])){
   ...
   $heslo = $_POST["heslo1"];
   $hesloZnovu = $_POST["heslo2"];

   if (empty($heslo) || empty($hesloZnovu)) {
     header("Location: ../phpStranky/vytvoreniNovehoHesla.php?hesla=prazdna");
     exit();
   }else if($heslo != $hesloZnovu){
     header("Location: ../phpStranky/vytvoreniNovehoHesla.php?rozdilna=hesla");
     exit();
   }
   else if(strlen($heslo) < 7){
		header("Location: ../phpStranky/vytvoreniNovehoHesla.php?error=heslojekratke");
		exit();
   }
   $dnesniDatum = date("U");
   ...
   if(!mysqli_stmt_prepare($stmt,$sql)){
     ...
     if (!$row = mysqli_fetch_assoc($vys)) {  //vezmeme data z vys a pridame je do asociativniho pole
       header("Location: ../index.php?updatehesla=zadnazadost");
       exit();
     }else{
       ...
       if ($tokenCheck === false) { //kontrolujeme jestli token check je true nebo false
         header("Location: ../index.php?updatehesla=notoken");
         exit();
         ...
         if(!mysqli_stmt_prepare($stmt,$sql)){
           ...
           if(!$row = mysqli_fetch_assoc($vys)){
             ...
             exit();
              ...
              $stmt = mysqli_stmt_init($pripojeni);
              if(!mysqli_stmt_prepare($stmt,$sql)){
                header("Location: ../index.php?updatehesla=problemupraveni");
                exit();
              }else{
                ...
                $stmt = mysqli_stmt_init($pripojeni);
                if(!mysqli_stmt_prepare($stmt,$sql)){
                  header("Location: ../index.php?updatehesla=problemdelete");
                  exit();
                }else{
                  mysqli_stmt_bind_param($stmt,"s",$tokenEmail);
                  mysqli_stmt_execute($stmt);
                  header("Location: ../index.php?updatehesla=uspesny");
                }
              }
           }
         }
