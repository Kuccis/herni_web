...
<title>Unit Roleplay - Twitch</title>
</head>
<body>
  ...
  <div class="container" id="textUvod">
		<div class="row">
      <h1 class="uvodVety" style="font-size: 70px;font-weight:bold;">Twitch</h1>
    </div>
    <div class="row">
      <h2 class="uvodVety" style="font-style: italic;">Roleplay is yours…</h2>
		...
		<br>
		<?php
		$sqli = "SELECT * FROM twitch ORDER BY id"; 	//limit - max int, dotazuju se na vypis vseho na urcitem id, ktere si beru z posledniho znaku v URL, kde si ho nastavuju podle id, když beru všechny řádky z datbaze
		$vysi = mysqli_query($pripojeni,$sqli);

		while($row = mysqli_fetch_assoc($vysi)){
			echo	'<div class="row" style="margin-left:0px!important;margin-right:0px!important;margin-bottom:20px;">
					    <div class="col-12 col-sm-6 col-md-4 col-lg-3">
					      <div class="our-team">
					        <div class="picture">';
									  $idUziv=$row['id'];
										$sqla = "SELECT * FROM uzivatele WHERE id=$idUziv;"; 	//limit - max int, dotazuju se na vypis vseho na urcitem id, ktere si beru z posledniho znaku v URL, kde si ho nastavuju podle id, když beru všechny řádky z datbaze
										$vysa = mysqli_query($pripojeni,$sqla);
										$rowa = mysqli_fetch_assoc($vysa);
									 if($rowa['profilovka'] == 0){
										 	$filename="../profilovka/profile".$row['id']."*";
										 	$fileinfo=glob($filename);
										 	$fileext=explode(".",$fileinfo[0]);
						          echo '<img class="img-fluid" src="../profilovka/profile'.$row['id'].'.'.$fileext[3].'" style="width:130px;height:130px;" alt="Profilová fotografie">';
									 }
									 else{
										 echo '<img class="img-fluid" src="../profilovka/default.jpg" style="width:130px;height:130px;" alt="Profilová fotografie">';
									 }
					  echo '</div>
					        <div class="team-content">
					          <h3 class="name">'.$row['jmeno'].'</h3>
					          <h4 class="title">Unit streamer</h4>
					        </div>
					        <ul class="social">
					          <li><a href="https://www.twitch.tv/'.$row['jmenoStreamu'].'/" target="_blank" aria-hidden="true"><i class="fab fa-twitch"></i></a></li>
					        </ul>
					      </div>
					    </div>
							...
							<iframe
						    src="https://player.twitch.tv/?channel='.$row['jmenoStreamu'].'&parent=unitrp.eu&parent=embeds.example.com"
						    height="460"
						    width="800"
						    frameborder="0"
						    scrolling="no"
