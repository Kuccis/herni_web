...
<title>Unit Roleplay - Editor základních pravidel</title>
</head>
<body>
  ...
  <!--KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL KONEC MODAL-->
	...
	if(isset($_SESSION['idUzivatele'])){
		...
		echo '<div class="container" id="textUvod">
		    <div class="row">
		      <h1 class="uvodVety" style="font-size: 70px;font-weight: bold;">Editor pravidel</h1>
		    </div>
		    <br>
		    <form class="editorPravidel" action="../skripty/upravitPravidla.php" method="post">';
        $sql = "SELECT * FROM zakladnipravidla";
    		$vys= mysqli_query($pripojeni,$sql);
  ...
  echo  '</textarea>
      ...
      <button type="submit" class="btn btn-secondary" data-dismiss="modal" name="submitUpravu" style="margin-top: 15px;">Upravit pravidla</button>
  </form>
  </div>
